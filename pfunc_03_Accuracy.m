...
実測値はそういうとても複雑なシステムから生じている可能性がある　全てをカバーするために、ここでは全ての実測値を用いて計算する
（つづく）
...
それぞれの行（ノード）のデータを独立に用いて計算する　上には independent MEM と書かれている
...
その値は、「その行（ノード）のみのデータから求めた確率を元にして計算した、ある活動パターンが出現する確率」
最初のセクションで求めた値は、データ全体から「ある活動パターンが出現する確率」を求めているので、
...
SN はすべての実測値から計算した「すべての相互作用を含めて考慮したエントロピー」
S1 は、それぞれのノード（行）のみの実測値から計算した「相互作用を考慮しないエントロピー」
