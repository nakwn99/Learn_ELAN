...
%%% この関数は、引数である二値化データ（実測値からの）をすべて一度に用いて、それに対応する確率 P_empirical、エントロピー（-p log P）を計算する
%%% また pfunc_02 で計算したパラメータ h, J を用いて、モデル(h, J) に対応する確率、エントロピーを計算する
%%% それには mfunc_StateProb(h,J)　を呼び出している
...
%%% 実測データからの確率 P_emp、エントロピーの計算を vectorList 　一列　ごとに繰り返している（行ではない）
...
%%% この下では、モデルのほうの確率 P_model(\sigma | h, J) を計算している　式 (5) は二つの項（単独、相互作用）がある二次モデルを示している　
...
%%% %% 二つの確率分布　p(X), q(X) があると、KL divergennce(p(X) | q(X)) = p(X) log (p(X) / q(X))
%%% %% p(X) と q(X) が完全に一致すれば、この値は 0 になる　　一致しないほど正の大きな値になる
%%% %% この場合 p(X) が probN データから計算した確率分布　q(X) が prob1 それぞれのノードのみの実測値から計算した確率分布   結果は D1
%%% %% または prob2 すべてのノードの相互作用を考慮した確率分布   結果は D2
%%% %% rD は (D1-D2)/D1　　この値は、ノード同士の相互作用がない場合 D1=D2 だから 0 に近づく　相互作用が大きければ D1 > D2 
%%% %%（D2 のほうが 0 に近い）になるので 正の大きな値になる
...
%%% 実測データからの確率（データ全体から計算・ノードごとに計算）とモデル(h, J)から計算した確率の一致度を計算している
%% Octave で確かめながらこの関数の機能を説明してみる
%% 資料：数理科学2019年6月号51ページ「エネルギー地形解析」増田直紀先生による解説　江崎先生による User's guide
