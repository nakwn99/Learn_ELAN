%%% この関数では、二値化した実測データからイジングモデルのパラメーター　h, J を推定している
%%% 参考資料：　数理科学2019年6月号51ページ　「エネルギー地形解析」増田直紀先生による解説
%%% 
%%% この関数では、二値化したデータからイジングモデルのパラメーター　h, J を推定している
%%% 増田先生の解説の式（５）にあるように、h はそれぞれのノードが単独でもつエネルギーに対する重み
%%% J はノード同士の相互作用エネルギーに対する重み
...
%%% 列が時間（時系列データ）　しかし解説の５３ページにあるように、データの順番を入れ替えても計算に影響はない
...
%%% この関数では、二値化した実測データからイジングモデルのパラメーター　h, J を推定している　oct2py でOK
%%% 参考資料：　数理科学2019年6月号51ページ　「エネルギー地形解析」増田直紀先生による解説
