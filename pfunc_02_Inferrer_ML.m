...
（行列の要素の 1,1 2,2 3,3 ...）は 0 にする（自分自身との相互作用は考えないので 0）ので、diag(diag(dJ)) を引き算して
対角成分 = 0 にしている
...
norm はベクトル・行列の長さ、大きさのようなもの　L1 ノルム、L2 ノルムとかたくさんの種類がある　'fro' を付けた場合、フロベニウス
ノルムと言うものを計算する　これは行列を対象として、すべての要素の二乗を足したものの平方根　sqrt(~ + ~) の部分は、dJ の長さと dh 
の大きさを二つの辺にした三角形のもう一つの辺の長さを計算しているように解釈できる。だから dJ, dh がバランスよく十分に小さくなれば
よいことになる
